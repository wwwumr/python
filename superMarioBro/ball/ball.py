...
class Block(Sprite):
    ...
    def getColor(self):
        ...
        for i in range(3):
            ...
            del base[index]
        return color
    def setPoly(self): #设置自身及拓展构成的多边形
...
def main(_root):
    ...
    stage.addChild(Sprite(canvas.create_image(0,0,
                          ...
                          0,0,WIDTH,HEIGHT))
    level = 30
    emitPos = (WIDTH/2,HEIGHT-R) #发射点位置
    blocks = []
    ball = None
...
def startGame(level):
    ...
    for block in blocks:
        block.setPoly()
    #开始侦听点击事件，点击后listener.run将变为False
    listener.run = True
    while True:
        sleep(INTERVAL)
...
def loop(pos,level):
    ...
    clockTime = clock() #clockTime用于计算程序运行时间
    frame = 0
    time = 0.0
    ...
    while True:
        ...
        if frame == 30: #每隔30帧输出一次fps
            sys.stdout.write("fps:%7.2f\r" %(frame/time))
            frame = time = 0
        if not accel: sleep(max(INTERVAL-runTime+clockTime,0)) #减去两次sleep之间经过的时间
        clockTime = clock()
